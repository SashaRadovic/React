...
export default function Element() {
    ...
    const handleCreateDiv = ()=> {
      ...
      setNumberBox(numberBox.concat(<Card name={numberOnCard} key={numberBox.length} />))
      if(yourScore>21){compWin=<p id='compwin'>Comp wins!</p>}
  }
    ...
    //setNumberBoxHold(numberBoxHold => [...numberBoxHold, <Card name={numberOnCard} key={Math.floor(Math.random()*1000)} />])
      numberBoxHoldCopy.push(<Card name={numberOnCard} key={Math.floor(Math.random()*1000)} />)
      ...
      else if(compScore>=yourScore&&compScore<22&&compScore>0){compWin=<p id='compwin'>Comp wins!</p>}
 
      else if (compScore>21){youWin=<p id='youwin'>You Win!</p>}
    ...
    return(
      <div className="wrap">
        ...
        <button id ="comp" onClick={handleCreateDivHold} >Hold</button>
